...
exports.reconstructProofFromCard = exports.requireAmount = exports.requireKeysetV0 = exports.requirePoint = exports.requireHex = void 0;
 ...
 */
const requireHex = (value, bytes, field, where = "") => {
    ...
    if (v.length !== bytes * 2) {
        throw rejection(field, `${where}${field} must be ${bytes} bytes (${bytes * 2} hex chars), got ${v.length}`);
    }
    return v;
};
 ...
 */
const requirePoint = (value, field, where = "") => {
    const bytes = Buffer.from(value, "hex");
    if (bytes[0] !== 0x02 && bytes[0] !== 0x03) {
        throw rejection(field, `${where}${field} must be a compressed secp256k1 point, got prefix 0x${value.slice(0, 2)}`);
    }
    if (!secp.isPoint(bytes)) {
        throw rejection(field, `${where}${field} is not on the secp256k1 curve: ${value}`);
    }
};
exports.requirePoint = requirePoint;
/**
 ...
 */
const requireKeysetV0 = (keysetId, where = "") => {
    if (!keysetId.startsWith("00")) {
        throw rejection("keysetId", `${where}keysetId must be a NUT-02 v0 id (00 version byte), got 0x${keysetId.slice(0, 2)}`);
    }
};
exports.requireKeysetV0 = requireKeysetV0;
/**
 ...
 */
const requireAmount = (value, where = "") => {
    ...
    if (!Number.isSafeInteger(value) || value <= 0 || Math.log2(value) % 1 !== 0) {
        ...
        `${where}amount must be a positive power of two, got ${Number.isFinite(value) ? JSON.stringify(value) : String(value)}`);
    }
    return value;
};
...
const reconstructProofFromCard = (slot, cardPubkey, options = {}) => {
    ...
    const pubkey = (0, exports.requireHex)(cardPubkey, 33, "cardPubkey");
    (0, exports.requirePoint)(pubkey, "cardPubkey");
    ...
    const keysetId = (0, exports.requireHex)(slot.keysetId, 8, "keysetId");
    const nonce = (0, exports.requireHex)(slot.nonce, 32, "nonce");
    const C = (0, exports.requireHex)(slot.C, 33, "C");
    (0, exports.requireKeysetV0)(keysetId);
    (0, exports.requirePoint)(C, "C");
    (0, exports.requireAmount)(slot.amount);
    return {
        id: keysetId,
        amount: slot.amount,
